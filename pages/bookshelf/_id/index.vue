<template>
  ...
  <tr>
    ...
    <td>{{ formatDate(book.published) }}</td>
            ...
            <div v-if="showRating" class="rating mt-2">
              <h4 class="fs-18">
              Student Rating and Reviews</h4>
              <div class="d-flex align-items-center">
                <div class="mt-5 mr-4">
                <span class="font-weight-600 rating-value">{{ averageRating ? averageRating.toFixed(1) : '0.0' }}</span>
                <star-rating :show-rating="false" :max-rating="5" :star-size="12" :padding="5" :read-only="true" v-model="averageRating" active-color="#FF9C09" :increment="0.5"></star-rating>
                </div>
                <div class="flex-grow-1 mt-3">
                  <div class="d-flex align-items-center">
                  <div class="text-grey mr-2 fs-14">5</div>
                  <b-progress :value="reviews.filter((el) => el.rating === 5).length" :max="reviews.length" height="8px" variant="warning" class="flex-grow-1 slide-in"></b-progress>
                </div>
                <div class="d-flex align-items-center">
                  <div class="text-grey mr-2 fs-14">4</div>
    <b-progress :value="reviews.filter((el) => el.rating === 4).length" :max="reviews.length" height="8px" variant="warning" class="flex-grow-1"></b-progress>
    </div>
    <div class="d-flex align-items-center">
                  <div class="text-grey mr-2 fs-14">3</div>
    <b-progress :value="reviews.filter((el) => el.rating === 3).length" :max="reviews.length" height="8px" variant="warning" class="flex-grow-1"></b-progress>
    </div>
    <div class="d-flex align-items-center">
                  <div class="text-grey mr-2 fs-14">2</div>
    <b-progress :value="reviews.filter((el) => el.rating === 2).length" :max="reviews.length" height="8px" variant="warning" class="flex-grow-1"></b-progress>
    </div>
    <div class="d-flex align-items-center">
                  <div class="text-grey mr-2 fs-14">1</div>
    <b-progress :value="reviews.filter((el) => el.rating === 1).length" :max="reviews.length" height="8px" variant="warning" class="flex-grow-1"></b-progress>
    </div>
                </div>
              </div>
              <div class="mt-4 review-details">
              <button class="btn btn-light btn-abs p-2 fs-12" @click.prevent="$bvModal.show('reviewBook')">Write a Review</button>
      ...
      :no-footer="true"
        ...
        <div class="col-md-5">
          ...
          <star-rating :show-rating="false" :max-rating="5" :star-size="28" :padding="5" v-model="selectedRating" active-color="#FF9C09"></star-rating>
        </div>
        </div>
        <div class="mt-3">
          <small>Review</small>
        ...
        <div class="d-flex justify-content-end my-3">
          <button class="btn btn-light px-4 py-2 mr-2" @click.prevent="$bvModal.hide('reviewBook')">Cancel</button>
          <button class="btn btn-primary px-4 py-2" @click.prevent="submitReview">Submit</button>
        </div>
        </ModalWrapper>
    </div>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      async getReview() {
        try {
          const { data } = await this.$axios.get(`/app/book/${this.$route.params.id}/reviews/`)
          this.reviews = data.data
          this.averageRating = this.reviews.reduce((sum, rating) => sum + rating.rating, 0) / this.reviews.length;
        } catch (error) {
          console.log(error)
        }
      },
      async addToBookshelf () {
        try{
          this.isLoading = true
        const response = await this.$axios.get(`/app/book/${this.$route.params.id}/add_to_shelf`)
        if (response.status === 200) {
          this.$toast({
            type: 'success',
